...
sprite_d::sprite_d(ID3D11Device* device, const wchar_t* filename)
{
    // 頂点バッファオブジェクトの生成
    HRESULT hr{ S_OK };
    ...
    create_vs_from_cso(device, "Shader/sprite_dissolve_vs.cso", vertex_shader.GetAddressOf(), input_layout.GetAddressOf(), input_element_desc, _countof(input_element_desc));
    create_ps_from_cso(device, "Shader/sprite_dissolve_ps.cso", pixel_shader.GetAddressOf());

    load_texture_from_file(device, filename, shader_resource_view.GetAddressOf(), &texture2d_desc);
}

void sprite_d::render(ID3D11DeviceContext* immediate_context,
...
void sprite_d::render(ID3D11DeviceContext* immediate_context, float dx, float dy, float dw, float dh, float r, float g, float b, float a, float angle, float sx, float sy, float sw, float sh)
{
...
void sprite_d::render(ID3D11DeviceContext* immediate_context, float dx, float dy, float dw, float dh)
{
...
void sprite_d::textout(ID3D11DeviceContext* immediate_context, std::string s, float x, float y, float w, float h, float r, float g, float b, float a)
{
